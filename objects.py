...
# creates background
class World:
	def __init__(self, surface, data, groups):
		self.surface = surface
		self.tile_size = 50
		self.tiles = []
		for t in sorted(os.listdir('tiles/'), key=lambda s: int(os.path.splitext(s)[0]) if os.path.splitext(s)[0].isdigit() else 0):
			self.tiles.append(pygame.image.load(f'tiles/{t}'))
		...
		for row in data:
			...
			self.tiles_list.append(row_tiles)
			...
			for col in row:
				if col > 0:
					if col in range(1,14) or col == 18:
						# dirt blocks
						img = pygame.transform.scale(tile[col-1], (tile_size, tile_size))
						rect = img.get_rect()
...
class Player:
	...
	def update(self, pressed_keys, game_over, level_won, game_won):
		...
		if not game_over and not game_won:
			...
			# check for colision
			self.in_air = True
			for tile in self.world.tiles_list:
...
def load_level(level):
    ...
    if os.path.exists(game_level):
        try:
            ...
            print(f"Error loading level {level}. Returning empty data.")
            return []
    else:
        print(f"Level {level} not found. Returning empty data.")
        return []
